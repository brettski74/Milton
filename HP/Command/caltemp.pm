package HP::Command::caltemp;

use strict;
use warnings qw(all -uninitialized);

use base qw(HP::Command::StateMachineCommand);
...
sub postprocess {
  ...
  my $interface = $self->{interface};
  my $filename = $config->{'output-file'} || 'caltemp.csv';
  my $fh = $self->replaceFile($filename);

  $fh->print("# Generated by HP::Command::caltemp at " . $self->timestamp . "\n");
...
sub _setupSteady {
  ...
  $self->{'steady-state'} = HP::SteadyStateDetector->new(%{$self->{config}->clone('steady-state')});
