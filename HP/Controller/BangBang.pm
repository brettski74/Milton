package HP::Controller::BangBang;
...
HP::Controller::BangBang - Bang-bang controller for hotplate temperature control

=head1 SYNOPSIS

    use HP::Controller::BangBang;
    
    my $controller = HP::Controller::BangBang->new($config, $interface);
    my $power = $controller->getRequiredPower($status, $target_temp);

=head1 DESCRIPTION

HP::Controller::BangBang implements a simple bang-bang (on/off) control strategy
...
This class inherits from L<HP::Controller::RTDController>, which provides:
...
Returns a new HP::Controller::BangBang instance.
...
=head1 USAGE EXAMPLE
    ...
    # Create interface
    my $interface = HP::Interface::DPS->new($dps_config);
...
The controller inherits configuration from L<HP::Controller::RTDController>.
...
=item * L<HP::Controller::RTDController> - Base class for RTD-based controllers

=item * L<HP::Controller::FeedForward> - Alternative controller with feed-forward compensation

=item * L<HP::Controller> - Controller base class
