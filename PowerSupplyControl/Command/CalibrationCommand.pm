...
use PowerSupplyControl::ValueTools qw(timestamp);
...
sub new {
  ...
  # Let's cheat and make our config object a Config object 
...
sub preprocess {
  ...
  if (!defined $self->{ambient}) {
    if ($self->{controller}->hasTemperatureDevice) {
      ...
      $self->info("Set ambient temperature to $self->{ambient}\n");
    } else {
...
=head2 writeCalibrationHeader($fh, %attributes)

=cut

sub writeCalibrationHeader {
  my ($self, $fh, @attributes) = @_;

  $fh->print('# Generated by '. ref($self) .' at '. timestamp ."\n");

  my $powerSupplyName = $self->{interface}->deviceName;
  my $sensorName = $self->{controller}->getDeviceName;

  _dual_print($fh, "# power-supply: $powerSupplyName\n") if $powerSupplyName;
  _dual_print($fh, "# temperature-sensor: $sensorName\n") if $sensorName;

  while (@attributes) {
    my $key = shift @attributes;
    my $data = shift @attributes;
...
=head2 writeCalibration($fh, $parent_key, $data, @keys)
...
The file handle to write the calibration data to.

=item $parent_key

The parent key for the calibration data.

=item $data

A reference to an array of hashes containing the calibration data.

=item @keys

An optional list of keys to be written out for each point in the data array. If not provided,
all keys in each has will be written out.

If keys are specified, rows must contain a value for all keys otherwise the row will be skipped.

=cut

sub writeCalibration {
  my ($self, $fh, $parent_key, $data, @keys) = @_;
  my $nokeys = 0;
  ...
  foreach my $point (@$data) {
    ...
    foreach my $key (@keys) {
      _dual_print($fh, "  $prefix$key: $point->{$key}\n");
      $prefix = '  ';
    }
  }
...
=head2 writeArrayCalibration($fh, $parent_key, $data, @keys)

Write out the calibration data to the provided file handle.

=cut

sub writeArrayCalibration {
  my ($self, $fh, $parent_key, $data, @keys) = shift;
  ...
  foreach my $point (@$data) {
    ...
    for (my $i=0; $i < @keys; $i++) {
      _dual_print($fh, "  $prefix$keys[$i]: $point->[$i]\n");
      $prefix = '  ';
    }
  }
