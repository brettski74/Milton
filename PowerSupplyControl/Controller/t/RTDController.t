...
subtest 'constructor with 2-point calibration' => sub {
  ...
  my $config = { calibration => { temperatures => [ { resistance => 1.0, temperature => 25.0 }
                                                , { resistance => 2.0, temperature => 225.0 }
                                                ]
                              }
             };

  my $rtd_controller = PowerSupplyControl::Controller::RTDController->new($config, $interface);
...
subtest 'one point estimator' => sub {
  my $config = { calibration => { temperatures => [ { resistance => 1.2, temperature => 25.0 } ] } };
  my $rtd_one = PowerSupplyControl::Controller::RTDController->new($config, $interface);
