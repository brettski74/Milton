package Milton::Controller::FeedForward;

use strict;
use Carp qw(croak);
use base qw(Milton::Controller::RTDController);
use Milton::Math::ThermalModel;

=head1 NAME

Milton::Controller::FeedForward - Implements a FeedForward controller that uses a thermal model to predict the power required to reach the next target temperature.
