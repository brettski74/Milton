...
subtest 'Zero Hysteresis, Flat' => sub {
  ...
  isa_ok($controller, 'Milton::Controller::BangBang');
  isa_ok($controller, 'Milton::Controller::RTDController');
  isa_ok($controller, 'Milton::Controller');
...
subtest 'Flat with Hysteresis' => sub {
  ...
  $controller = Milton::Controller::BangBang->new($config, $interface);
...
subtest 'Cut-off Temperature' => sub {
  my $config = { limits => { 'cut-off-temperature' => 225 }
               , hysteresis => { low => 0, high => 0 }
               , predictor => { package => 'Milton::Predictor::LossyLPF'
                             , tau => 28
                             , 'loss-factor' => 1.0
                             }
               };
