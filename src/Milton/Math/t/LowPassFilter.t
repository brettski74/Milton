...
use Test2::V0;
use Milton::Math::LowPassFilter;

# Tolerance for floating point comparisons
my $EPS = 1e-6;

subtest 'Basic static filtering' => sub {
  my $f= Milton::Math::LowPassFilter->new(tau => 9, period => 1);
    
  isa_ok($f, 'Milton::Math::LowPassFilter');
...
subtest 'Constructor validation' => sub {
  ...
  like(dies { Milton::Math::LowPassFilter->new(period => 0.1); }
     , qr/tau must be non-negative/, 'Requires tau'
     );

  like(dies { Milton::Math::LowPassFilter->new(tau => 1.0); }
     , qr/period must be positive/, 'Requires period'
     );

  like(dies { Milton::Math::LowPassFilter->new(tau => -1, period => 1); } 
     , qr/tau must be non-negative/, 'Requires non-negative tau'
     );

  like(dies { Milton::Math::LowPassFilter->new(tau => 1, period => 0); } 
     , qr/period must be positive/, 'Requires non-zero period'
     );

  like(dies { Milton::Math::LowPassFilter->new(tau => 1, period => -2); } 
...
subtest 'Edge cases - zero tau' => sub {
  ...
  my $f = Milton::Math::LowPassFilter->new(tau => 0.0, period => 0.1);
