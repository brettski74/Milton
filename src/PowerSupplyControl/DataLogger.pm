package PowerSupplyControl::DataLogger;
...
PowerSupplyControl::DataLogger - A data logger for the hotplate controller.
...
sub new {
  ...
  if (!$self->{enabled} || lc($self->{enabled}) eq 'false') {
    ...
    bless $self, 'PowerSupplyControl::DataLogger::Null';
  } else {
    bless $self, $class;
