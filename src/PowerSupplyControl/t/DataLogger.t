...
my $logger = PowerSupplyControl::DataLogger->new($config);
isa_ok($logger, 'PowerSupplyControl::DataLogger');
...
my $null_logger = PowerSupplyControl::DataLogger->new({ enabled => 0 });
isa_ok($null_logger, 'PowerSupplyControl::DataLogger::Null');
...
my $tee_logger = PowerSupplyControl::DataLogger->new($tee_config);
...
my $no_tee_logger = PowerSupplyControl::DataLogger->new($no_tee_config);
